...
var casparMsg = {
	'100': 'Information about an event.', // *
	'101': 'Information about an event. A line of data is being returned.', // *
	'200': 'The command has been executed and several lines of data (seperated by \\r\\n) are being returned (terminated with an additional \\r\\n)',// *
	'201': 'The command has been executed and data (terminated by \\r\\n) is being returned.', // *
	'202': 'The command has been executed.', // *
	'400': 'ERROR - Command not understood.',
	'401': 'ERROR - Illegal video_channel',
	'402': 'ERROR - Parameter missing',
	'403': 'ERROR - Illegal parameter',
	'404': 'ERROR - Media file not found.',
...
module.exports = function (ccg) {
	...
	ccg.prototype.reconnecting = false;
	...
	ccg.prototype.connect = function (host, port, cb) {
		...
		client.on("connectionTimeout", function (err) {
			if (cb) {
				cb(err);
			}
			self.emit("connectionError", err);
		...
		client.on("data", function (chunk) {
			...
			if (self.responseCode >= 400) {
				// callback with error
				if (cb) {
					var error = new Error(casparMsg[self.responseCode]);
					error.code = self.responseCode
					cb(error);
				}
