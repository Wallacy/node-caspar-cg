...
module.exports = function (ccg) {
	...
	ccg.prototype.getTemplateInfo = function (file, cb) {
		...
		self.sendCommand("INFO TEMPLATE \"" + file.replace("\\", "\\\\") + "\"", function (err, data) {
			...
			for (var i in parts) {
				part = /[\s]*<instance\sname="([^"]*)"\stype="([^"]*)"\/>/.exec(parts[i]);

				if (part) {
					data.fields.push({name: part[1], type: part[2]});
				}

				part = /[\s]*<parameter\sid="([^"]*)"/.exec(parts[i]);

				if (part) {
					data.fields.push({name: part[1]});
				}
			}
			...
			if (data.substr(0, 1) == "<") {
				xml(data, function (err, data) {
					...
					var processLayer = function (layer) {
						...
						if (layer.foreground) layer.foreground = processProducer(layer.foreground.producer);
						if (layer.background) layer.background = processProducer(layer.background.producer);
	...
	//
	// [INFO](http://casparcg.com/wiki/CasparCG_2.0_AMCP_Protocol#INFO_Examples)
	ccg.prototype.infoSystem = function (cb) {
		var self = this;
		self.sendCommand("INFO SYSTEM", function (err, data) {
			if (err) {
				cb(err);
				return;
			}
	...
	//
	// [INFO](http://casparcg.com/wiki/CasparCG_2.0_AMCP_Protocol#RESTART)
	ccg.prototype.restart = function (cb) {
		var self = this;
		self.sendCommand("RESTART", function (err, data) {});
		if (cb) cb(null, null);
	};
};
